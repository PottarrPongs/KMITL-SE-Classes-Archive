...
#define SIZE 2000 // Adjustable matrix size
    ...
    for (int i = start_row; i < end_row; i += BLOCK_SIZE) {
      for (int j = 0; j < SIZE; j += BLOCK_SIZE) {
        for (int k = 0; k < SIZE; k += BLOCK_SIZE) {
          // Block multiplication with loop unrolling
          for (int ii = i; ii < i + BLOCK_SIZE && ii < end_row; ii++) {
            for (int jj = j; jj < j + BLOCK_SIZE && jj < SIZE; jj++) {
              int sum = result[ii][jj];
              int kk;
              for (kk = k; kk <= k + BLOCK_SIZE - 4 && kk + 3 < SIZE; kk += 4) {
                sum += matrixA[ii][kk]     * matrixB[kk][jj]
                  + matrixA[ii][kk + 1] * matrixB[kk + 1][jj]
                  + matrixA[ii][kk + 2] * matrixB[kk + 2][jj]
                  + matrixA[ii][kk + 3] * matrixB[kk + 3][jj];
              }
              for (; kk < k + BLOCK_SIZE && kk < SIZE; kk++) {
                sum += matrixA[ii][kk] * matrixB[kk][jj];
              }
              result[ii][jj] = sum;
            }
